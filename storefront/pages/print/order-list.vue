<template>
  <div>
    <div id="inv_item" :key="o_index" v-for="(order, o_index) in order_list">
      <section v-if="order" class="section container my-0 py-0">
        ...
        <col span="50%" />
          ...
          <thead>
            ...
            <th
              v-if="
                order.item[0].discount != 0 && order.item[0].discount != null
              "
            >
              Discount
            </th>
          ...
          <tbody>
            <tr :key="index" v-for="(item, index) in order.item">
                ...
                {{
                  (
                    parseFloat(
                      (item.price -
                        parseFloat(
                          item.price / ((100 + Number(item.tax)) / 100)
                        )) /
                        2
                    ) * item.qty
                  ).toFixed(2)
              ...
              <td>
                {{
                  (getDiscountRate(item) * item.qty * (item.tax / 100)).toFixed(
                    2
                  )
                }}
              </td>
            ...
            <tr v-if="order.shipping_rate != 0 && order.shipping_rate != null ">
              ...
              <td
                v-if="
                  order.order_type == 'india' || order.order_type == 'export'
                "
              >
                {{ order.shipping_tax }}
              </td>
...
export default {
  ...
  mounted() {
    ...
    let temp_list = JSON.parse(localStorage.getItem("order-list"));
    temp_list.forEach((item) => {
      console.log(item);
      this.$axios.get("/get/order/" + item.order_number).then((response) => {
  ...
  methods: {
    ...
    getTotalRate(index) {
      let total = 0.0;
      this.order_list[index].item.forEach((item) => {
        total +=
    ...
    getTotalTaxes(index) {
      ...
      if (this.order_list[index].shipping_rate != 0) {
        total +=
          this.order_list[index].shipping_rate -
          parseFloat(
            this.order_list[index].shipping_rate /
              ((100 + Number(this.order_list[index].shipping_tax)) / 100)
          );
      }
      return parseFloat(total).toFixed(2);
    },
    getTotalTaxable(index) {
      let total = 0.0;
      this.order_list[index].item.forEach((item) => {
