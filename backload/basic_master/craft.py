...
from model import Craft, CraftSchema
from app import db, ma
import app
import json


@app.route('/get/craft', methods=['GET'])
def get_uom():

        data_schema = CraftSchema(many=True)
        data = Craft.query.order_by(Craft.name).all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@app.route('/add/craft', methods=['POST'])
def add_uom():
        payload = request.json
        if len(payload['name']) != 0:

            check_data = Craft.query.filter_by(
...
@app.route('/edit/craft', methods=['POST'])
def edit_uom():
        ...
        if payload['name'] is not None:
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = Craft.query.filter_by(
                        id=payload['id']).first()
...
@app.route('/delete/craft', methods=['POST'])
def delete_uom():
        payload = request.json
        check_data = Craft.query.filter_by(id=payload['id'])
        if check_data.first():
