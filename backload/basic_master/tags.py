...
from model import Tags, TagsSchema
...
def get_uom():
        ...
        data_schema = TagsSchema(many=True)
        data = Tags.query.order_by(Tags.name).all()
...
def add_uom():
        ...
        if len(payload['name']) != 0:
            ...
            check_data = Tags.query.filter_by(
...
def delete_uom():
        payload = request.json
        check_data = Tags.query.filter_by(id=payload['id'])
        if check_data.first():
