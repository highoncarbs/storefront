...
from model import Colour, ColourSchema
...
def get_uom():
        ...
        data_schema = ColourSchema(many=True)
        data = Colour.query.order_by(Colour.name).all()
        json_data = data_schema.dump(data)
        return jsonify(json_data)


@app.route('/add/colour', methods=['POST'])
def add_uom():
        payload = request.json
        if len(payload['name']) != 0:

            check_data = Colour.query.filter_by(
...
@app.route('/edit/colour', methods=['POST'])
def edit_uom():
        ...
        if payload['name'] is not None:
            ...
            if check_data and check_data.name != payload['name'].lower().strip():
                ...
                try:
                    new_data = Colour.query.filter_by(
                        id=payload['id']).first()
...
@app.route('/delete/colour', methods=['POST'])
def delete_uom():
        payload = request.json
        check_data = Colour.query.filter_by(id=payload['id'])
        if check_data.first():
